...
1.  **好友/AI 角色 (Friends)**: 存储 AI 的人设信息，支持预设和自定义。
2.  **会话 (Sessions)**: 管理用户的聊天上下文，关联特定的人格。
3.  **消息 (Messages)**: 存储具体的对话内容，需记录所属会话及对应好友。
...
| name | varchar | 64 | NO | | 好友名称 (如 "编程助手") |
| description | varchar | 255 | YES | NULL | 好友简述 |
...
CREATE TABLE `friends` (
  ...
  `name` varchar(64) NOT NULL COMMENT '好友名称',
  `description` varchar(255) DEFAULT NULL COMMENT '好友描述',
  `system_prompt` text DEFAULT NULL COMMENT '系统提示词',
  `is_preset` tinyint(1) NOT NULL DEFAULT 0 COMMENT '是否为预设好友 0-否 1-是',
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| id | bigint | 20 | NO | AUTO_INCREMENT | 主键 |
| friend_id | bigint | 20 | NO | | 关联的默认好友ID |
...
CREATE TABLE `chat_sessions` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `friend_id` bigint(20) NOT NULL COMMENT '关联的好友ID',
...
| friend_id | bigint | 20 | YES | NULL | 关联的好友ID (快照或特定发言人) |
...
## 3. 关系说明
- **Friends - ChatSessions**: 一对多。一个好友可以发起多个会话。
- **Friends - Messages**: 一对多。一条消息（特别是 assistant 消息）关联到一个特定的好友。
- **ChatSessions - Messages**: 一对多。一个会话包含多条消息。

## 4. 备注与建议
- **索引**: 针对查询频繁 of `session_id` 和 `friend_id` 建立了普通索引。
- **扩展性**: `messages` 表增加 `friend_id` 使得在同一会话中切换角色或多角色对话成为可能。
