...
#### B. `system_settings` 表（新增）
用于统一存储运行时可调配置，采用 **分组 Key-Value** 模式：
...
配置项存储于 `system_settings` 表，采用 `group_name.key` 命名规范：
...
| 模块 | 功能 | 相关文件 | 备注 |
| :--- | :--- | :--- | :--- |
| **数据库** | `chat_sessions` 表扩展 (`memory_generated`, `last_message_time`) | `server/app/models/chat.py` | 迁移 `7fa2b40a0479` |
| **数据库** | `system_settings` 表 (分组 KV 配置存储) | `server/app/models/system_setting.py` | 迁移 `0a8be336eb20` |
| **后端服务** | `SettingsService` (配置读写、类型转换、默认值初始化) | `server/app/services/settings_service.py` | 支持 int/bool/float/json/string |
...
| 模块 | 功能 | 相关文件 | 备注 |
| :--- | :--- | :--- | :--- |
| **前端 - 设置界面** | "记忆设置" 选项卡 | `front/src/components/SettingsDialog.vue` | 支持配置 `session.passive_timeout` |
| **前端 - 设置 API** | Settings API 封装 | `front/src/api/settings.ts` | 调用 `/api/settings` 接口 |
| **前端 - 设置 Store** | Settings 状态管理 | `front/src/stores/settings.ts` | 分钟/秒转换便捷方法 |
| **前端 - 会话边界** | 时间戳分隔线 | `front/src/components/ChatArea.vue` | 检测 `session_id` 变化自动显示 |
...
│   ├── services/
│   │   ├── chat_service.py      # 被动会话逻辑
│   │   └── settings_service.py  # 配置服务
│   └── db/
│       └── init_db.py           # 启动时初始化默认配置
└── alembic/versions/
    ├── 7fa2b40a0479_*.py        # chat_sessions 扩展
    ├── 0a8be336eb20_*.py        # system_settings 表
    └── fix_001_*.py             # 索引修复

front/
└── src/
    ├── api/
    │   └── settings.ts          # Settings API 封装
    ├── stores/
    │   └── settings.ts          # Settings 状态管理
    └── components/
        ├── SettingsDialog.vue   # 新增"记忆设置"选项卡
        └── ChatArea.vue         # 会话边界时间戳分隔线
```
